...
import { FlowConnectState, SerializedNode } from "flow-connect";
...
export class Source extends Node {
  ...
  constructor(flow: Flow, options: NodeCreatorOptions = {}) {
    super(
      ...
      options.width || 200,
      [
        { name: "array-buffer", dataType: "event" },
        { name: "gain", dataType: "audioparam" },
        { name: "detune", dataType: "audioparam" },
        { name: "playback-rate", dataType: "audioparam" },
      ],
      [
        { name: "out", dataType: "audio" },
        { name: "ended", dataType: "event" },
      ],
      {
        state: options.state
          ? { ...Source.DefaultState, ...options.state }
          : Source.DefaultState,
    ...
    this.inputs[2].ref = (
      this.proxyParamSourceNode.parameters as Map<string, AudioParam>
    ).get("detune");
    this.inputs[3].ref = (
      this.proxyParamSourceNode.parameters as Map<string, AudioParam>
    ).get("playbackRate");
    this.inputs[1].on(
      "data",
      (_, data) => typeof data === "number" && (this.inputs[1].ref.value = data)
    );
    this.inputs[2].on(
      "data",
      (_, data) => typeof data === "number" && (this.inputs[2].ref.value = data)
    );
    this.inputs[3].on(
      "data",
      (_, data) => typeof data === "number" && (this.inputs[3].ref.value = data)
    );
    ...
    this.inputs[0].on("event", (_, data) => {
      if (!(data instanceof ArrayBuffer)) {
        Log.error(
          "Data received on Audio Source Node is not of type ArrayBuffer"
        );
        return;
      }
      this.processArrayBuffer(data);
    });
    this.fileInput.on("change", (_inst, _oldVal: File, newVal: File) =>
      this.processFile(newVal)
    );
    this.fileInput.on("upload", (_inst, _oldVal: File, newVal: File) =>
      this.processFile(newVal)
    );
  ...
  async processFile(file: File) {
    let cached = this.flow.flowConnect.arrayBufferCache.get(
      file.name + file.type
    );
    if (!cached) {
  ...
  async processArrayBuffer(arrayBuffer: ArrayBuffer) {
    ...
    if (!cached) {
      cached = await this.flow.flowConnect.audioContext.decodeAudioData(
        arrayBuffer
      );
  ...
  playSource() {
    if (
      this.flow.flowConnect.state === FlowConnectState.Stopped ||
      !this.state.buffer
    )
      return;

    let audioSource = new AudioBufferSourceNode(
      this.flow.flowConnect.audioContext
    );
  ...
  setupUI() {
    this.fileInput = this.createSource({
      input: true,
      output: true,
      height: 25,
      style: { grow: 0.7 },
    });
    this.loopToggle = this.createToggle({
      propName: "loop",
      input: true,
      output: true,
      height: 10,
    ...
    this.ui.append([
      this.createHozLayout(
        [this.createLabel("File", { style: { grow: 0.3 } }), this.fileInput],
        { style: { spacing: 5 } }
      ),
      this.createHozLayout([this.createLabel("Loop ?"), this.loopToggle], {
        style: { spacing: 5 },
      }),
    ]);
  }
  handleAudioConnections() {
    ...
    this.outputs[0].on("connect", (_, connector) => {
      this.state.buffer &&
        this.propagateChannelChange(this.state.buffer.numberOfChannels);
